    ...
    : Controller
{
    ...
    public async Task<IActionResult> Signup(SignupViewModel model)
    {
        ...
        {
            //Checking whether the role is already out there
            if (!await roleManager.RoleExistsAsync(model.Role))
            {
                var role = new IdentityRole { Name = model.Role };
                //If not adding into the DB
                var roleResult = await roleManager.CreateAsync(role);
                if (!roleResult.Succeeded)
                {
                    //Some issue we will show the general message 
                    //as per the recomendation from the tutor i should not show the 
                    //precise Error message it will help the hacker to hack it
            ...
            //If the email is not there in the db user will be created
